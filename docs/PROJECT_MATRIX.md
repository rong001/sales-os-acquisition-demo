...
| 获客（本仓库） | [已交付](https://github.com/rong001/sales-os-acquisition-demo) | 公网仍为 **临时** trycloudflare（以 `docs/acceptance/toc-prod/PUBLIC_URL.txt` 或 `toc-stable/PUBLIC_URL.txt` / `public-https/PUBLIC_URL.txt` 为准；重部署会换 URL）。**固定域名：待用户 DNS** | 生产静态 dist + gateway :18180 + supervise；Named Tunnel 配置/脚本已备（`docs/PROD_HTTPS.md`）但凭证/域名未装；Compose 有、本机无引擎则未实测 | **能力已交付（本机+临时公网可复测）**：落地页（演示≠正式合同 + 抢票/USGate 诚实边界）、SEO `/c/:slug`、UTM/邀请码/渠道短链 `/r/:code`、同意证据（含 hash/IP/UA/渠道）、限流 429、去重、分配/跟进/预约/**结果标记**、漏斗+来源转化、活动/渠道/邀请码管理、审计只读列表、viewer RBAC；触达 SMS/Call **MOCK**；邮件无 SMTP → **未送达**标签 | 公开页 / 同意追踪 / 持久流转 / 权限：可独立复测。**部署：固定域名未完成**（仅临时隧道）→ 整体获客标 **「能力已交付 / 固定域名待用户」** — **不要**写「生产稳定已完成」或「可售」 | Cloudflare 登录；域名 DNS CNAME；Named Tunnel 凭证；真实触达凭据；Compose 引擎 |
